...
**Total: 772 pages translated** (wave 16, 2026-09-03: +50 Next.js pages)
Wave history: W1=20 → W2=52 → W3=70 → W4=101 → W5=167 → W6=241 → W7=289 → W8=331 → W9=393 → W10=457 → W11=520 → W12=555 → W13=633 → W14=672 → W15=722 → W16=772
...
## Next.js (nextjs.org/docs app) — est ~300 — 169 done
...
- [ ] API reference ကျန်: next.config.js options အသေးစား ~30 ကျန် (cacheHandlers, cacheLife, deploymentId, expireTime, exportPathMap, incrementalCacheHandlerPath, inlineCss, instrumentationClientInject, optimizePackageImports, partialPrefetching, prefetchInlining, proxyClientMaxBodySize, reactCompiler, reactMaxHeadersLength, supportsImmutableAssets, taint, turbopack* ×6, useLightningcss, useOffline, useTypeScriptCli, adapterPath, appDir, cssChunking, htmlLimitedBots, optimizePackageImports, output, serverComponentsHmrCache done ✓ စသည်), api-reference functions/file-conventions အသေးစားများ (not-found function merge decision, useOffline), guides ကျန် (migrating/*, analytics done ✓, open-telemetry done ✓, third-party-libraries done ✓, testing suites (cypress/jest/playwright/vitest), self-hosting done ✓, single-page-applications done ✓, progressive-web-apps, ai-agents done ✓, mcp done ✓, data-security, content-security-policy, interactive-apps, backend-for-frontend, offline-support, preserving-ui-state, instant-navigation, package-bundling, scripts, videos, building, server-actions, authentication-with-cache-components, caching-without-cache-components, incremental-static-regeneration(+cc), local-development done ✓, memory-usage done ✓) — prose pages link to official nextjs.org URLs pending translation
