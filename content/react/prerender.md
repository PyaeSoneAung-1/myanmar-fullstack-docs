---
title: "prerender"
description: "React tree တစ်ခုကို Web Stream သုံးပြီး static HTML string အဖြစ် render လုပ်ပေးတဲ့ react-dom/static API — bootstrapScripts/bootstrapModules/identifierPrefix စတဲ့ options များ၊ prelude/postponed returns များ၊ SSG အတွက် data အားလုံး load ပြီးမှ resolve လုပ်ခြင်းနဲ့ prerendering ကို abort လုပ်ခြင်း"
order: 109
...
`prerender` က [Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) တစ်ခုသုံးပြီး React tree တစ်ခုကို static HTML string အဖြစ် render လုပ်ပေးပါတယ်။

```js
const {prelude, postponed} = await prerender(reactNode, options?)
```

> **မှတ်ချက်:** ဒီ API က [Web Streams](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) ပေါ်မှာ မူတည်ပါတယ်။ Node.js အတွက်ဆိုရင် — [`prerenderToNodeStream`](/docs/react/prerender-to-node-stream) ကို သုံးပါ။

## ရည်ညွှန်းချက် (Reference)

### `prerender(reactNode, options?)`

သင့် app ကို static HTML အဖြစ် render လုပ်ဖို့ `prerender` ကို ခေါ်ပါ:
...
- **optional** `options`: Static generation options တွေ ပါတဲ့ object တစ်ခု။
  ...
  - **optional** `onError`: Server error တစ်ခု ဖြစ်တိုင်း — [recoverable](https://react.dev/reference/react-dom/server/renderToReadableStream#recovering-from-errors-outside-the-shell) ဖြစ်ဖြစ် [မဟုတ်ဘဲ](https://react.dev/reference/react-dom/server/renderToReadableStream#recovering-from-errors-inside-the-shell) ဖြစ်ဖြစ် — fire ဖြစ်တဲ့ callback တစ်ခု။ ပုံမှန်အားဖြင့် ဒါက `console.error` ကိုပဲ ခေါ်ပါတယ်။ [Crash reports တွေ log လုပ်ဖို့](https://react.dev/reference/react-dom/server/renderToReadableStream#logging-crashes-on-the-server) override လုပ်မယ်ဆိုရင် — `console.error` ကို ဆက်ခေါ်ဖို့ သေချာပါစေ။ Shell မထုတ်လွှတ်ခင်မှာ [status code ကို ချိန်ညှိဖို့](https://react.dev/reference/react-dom/server/renderToReadableStream#setting-the-status-code) လည်း သုံးနိုင်ပါတယ်။
...
`prerender` က Promise တစ်ခုကို ပြန်ပေးပါတယ်:
...
- Render လုပ်တာ အောင်မြင်ခဲ့ရင် — Promise က အောက်ပါတို့ ပါတဲ့ object တစ်ခုအဖြစ် resolve ဖြစ်ပါလိမ့်မယ်:
  ...
  - `postponed`: JSON serialize လုပ်လို့ရတဲ့ opaque object တစ်ခု — `prerender` က ပြီးမြောက်မသွားခဲ့ရင် [`resume`](/docs/react/resume) ဆီ ဒါကို ပေးပို့နိုင်ပါတယ်။ မဟုတ်ရင်တော့ — `prelude` ထဲမှာ content အားလုံး ပါဝင်ပြီး resume မလိုအပ်ဘူးဆိုတာကို ပြတဲ့ `null` ဖြစ်ပါတယ်။
...
> **မှတ်ချက် — `prerender` ကို ဘယ်အချိန်မှာ သုံးသင့်လဲ**
>
> Static `prerender` API ကို static server-side generation (SSG) အတွက် သုံးပါတယ်။ `renderToString` နဲ့ မတူဘဲ — `prerender` က resolve မဖြစ်ခင် data အားလုံး load ပြီးတဲ့အထိ စောင့်ပါတယ်။ ဒါကြောင့် — Suspense သုံးပြီး fetch လုပ်ဖို့ လိုတဲ့ data တွေ အပါအဝင် — page တစ်ခုလုံးအတွက် static HTML generate လုပ်ဖို့ သင့်တော်ပါတယ်။ Content တွေ load ဖြစ်လာတာနဲ့အမျှ stream လုပ်ချင်ရင် — [renderToReadableStream](/docs/react/render-to-readable-stream) လို streaming server-side render (SSR) API တစ်ခုကို သုံးပါ။
>
> `prerender` ကို abort လုပ်နိုင်ပြီး — နောက်မှ `resumeAndPrerender` နဲ့ ဆက်လုပ်နိုင်သလို — partial pre-rendering ကို support လုပ်ဖို့ `resume` နဲ့လည်း resume လုပ်နိုင်ပါတယ်။

## အသုံးပြုပုံ (Usage)

### React tree တစ်ခုကို static HTML stream အဖြစ် render လုပ်ခြင်း

သင့် React tree ကို [Readable Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream) တစ်ခုထဲကို static HTML အဖြစ် render လုပ်ဖို့ `prerender` ကို ခေါ်ပါ:
...
သင့် app ကို static HTML string အဖြစ် render လုပ်ဖို့ `prerender` ကို ခေါ်ပါ:
...
`prerender` က static HTML generation ကို အပြီးသတ်ပြီး resolve မဖြစ်ခင် — data အားလုံး load ပြီးတဲ့အထိ စောင့်ပါတယ်။ ဥပမာ — cover တစ်ခု၊ friends နဲ့ photos တွေပါတဲ့ sidebar တစ်ခုနဲ့ posts list တစ်ခု ပြတဲ့ profile page တစ်ခု ဆိုပါစို့:
