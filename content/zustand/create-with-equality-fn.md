---
title: "createWithEqualityFn (Custom Equality Check နဲ့ Store ဖန်တီးခြင်း)"
description: "createWithEqualityFn နဲ့ စိတ်ကြိုက် equality function သုံးပြီး store ဖန်တီးနည်း — re-render တွေကို ထိန်းချုပ်ခြင်း၊ state update လုပ်နည်းများနဲ့ troubleshooting"
order: 18
source: "https://zustand.docs.pmnd.rs/reference/apis/create-with-equality-fn"
status: translated
updated: 2026-09-01
---

`createWithEqualityFn` က `create` လိုပဲ — API utilities တွေ တွဲပါတဲ့ React Hook တစ်ခုကို ဖန်တီးပေးပါတယ်။ ဒါပေမယ့် စိတ်ကြိုက် equality check တစ်ခုကို သတ်မှတ်နိုင်တဲ့ နည်းလမ်းကိုလည်း ပေးထားပါတယ်။ ဒါက component တွေ ဘယ်အချိန်မှာ re-render ဖြစ်မလဲဆိုတာကို ပိုပြီး သေချာကျစေပြီး — performance နဲ့ responsiveness ကို မြှင့်တင်ပေးပါတယ်။

> **အရေးကြီး:** `zustand/traditional` ကနေ `createWithEqualityFn` ကို သုံးဖို့ — `use-sync-external-store` library ကို install လုပ်ထားဖို့ လိုပါတယ်။ ဘာလို့လဲဆိုတော့ `zustand/traditional` က `useSyncExternalStoreWithSelector` ကို မှီခိုနေလို့ပါ။
...
- **optional** `defaultEqualityFn`: Default အားဖြင့် `Object.is` ဖြစ်ပါတယ်။ Re-render တွေကို ကျော်သွားခွင့်ပြုပေးတဲ့ (skip) function တစ်ခုပါ။

#### Returns

`createWithEqualityFn` က `create` လိုပဲ — API utilities တွေ တွဲပါတဲ့ React Hook တစ်ခုကို ပြန်ပေးပါတယ်။ Selector function သုံးပြီး လက်ရှိ state ကို အခြေခံတဲ့ data တွေကို ပြန်ပေးနိုင်ပြီး — equality function သုံးပြီး re-render တွေကို ကျော်သွားနိုင်ပါတယ်။ Selector function တစ်ခုနဲ့ equality function တစ်ခုကို argument အဖြစ် လက်ခံပါတယ်။
...
> **အရေးကြီး:** Immutable operation တွေကို ဦးစားပေး သုံးသင့်ပါတယ် — `[...array]`, `concat(...)`, `filter(...)`, `slice(...)`, `map(...)`, `toSpliced(...)`, `toSorted(...)` နဲ့ `toReversed(...)` စတာတွေပေါ့။ `array[arrayIndex] = ...`, `push(...)`, `unshift(...)`, `pop(...)`, `shift(...)`, `splice(...)`, `reverse(...)` နဲ့ `sort(...)` စတဲ့ mutable operation တွေကိုတော့ ရှောင်ကြဉ်ပါ။
