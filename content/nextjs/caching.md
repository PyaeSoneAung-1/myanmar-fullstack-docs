...
Next config file ထဲမှာ [`cacheComponents`](https://nextjs.org/docs/app/api-reference/next-config-js/cacheComponents) option ထည့်ပြီး Cache Components ကို enable လုပ်နိုင်ပါတယ်:
...
[`use cache`](https://nextjs.org/docs/app/api-reference/directives/use-cache) directive က async functions နဲ့ components တွေရဲ့ return value တွေကို cache လုပ်ပါတယ်။ အဆင့် နှစ်မျိုးမှာ သုံးနိုင်ပါတယ်:
...
> **သိထားသင့်သည်** — Cache directive တိုင်းကို [`cacheLife`](https://nextjs.org/docs/app/api-reference/functions/cacheLife) နဲ့ တွဲသုံးဖို့ အကြံပြုပါတယ်။ မသုံးရင် implicit `default` profile ကို သုံးပါလိမ့်မယ်။

Arguments တွေနဲ့ parent scopes တွေကနေ ဖမ်းယူလိုက်တဲ့ (captured) value တွေက [cache key](https://nextjs.org/docs/app/api-reference/directives/use-cache#cache-keys) ရဲ့ အစိတ်အပိုင်း အလိုအလျောက် ဖြစ်သွားပါတယ် — ဆိုလိုတာက input မတူရင် cache entry သီးခြားစီ ဖြစ်ပါတယ်။ Entry တစ်ခုထဲမှာ ဘာတွေ ပါဝင်လဲဆိုတာကို [cache output](https://nextjs.org/docs/app/api-reference/directives/use-cache#cache-output) မှာ၊ ဘာတွေကို cache လုပ်နိုင်လဲ၊ arguments တွေ ဘယ်လို အလုပ်လုပ်လဲဆိုတဲ့ အသေးစိတ်ကို [serialization requirements and constraints](https://nextjs.org/docs/app/api-reference/directives/use-cache#constraints) မှာ ကြည့်ပါ။
...
- [`cookies`](https://nextjs.org/docs/app/api-reference/functions/cookies) — User ရဲ့ cookie data
...
- [`params`](https://nextjs.org/docs/app/api-reference/file-conventions/page#params-optional) — Dynamic route parameters — build time မှာ တိကျတဲ့ value တွေကို prerender လုပ်ဖို့ [`generateStaticParams`](https://nextjs.org/docs/app/api-reference/functions/generate-static-params) ကို သုံးပါ၊ (သို့) မသိတဲ့ params တွေက နောက်ခံမှာ ဖြေရှင်းနေချိန်မှာ [App Shell](https://nextjs.org/docs/app/glossary#app-shell) တစ်ခုကို ပေးဖို့ [Cache Components နဲ့ ISR](https://nextjs.org/docs/app/guides/incremental-static-regeneration) ကို သုံးပါ။
...
Runtime ပေါ်မှာ မှီခိုတဲ့ data ကိုလည်း Cache Components နဲ့အတူ ပါလာတဲ့ နောက်ထပ် variant တစ်ခုဖြစ်တဲ့ [`"use cache: private"`](https://nextjs.org/docs/app/api-reference/directives/use-cache) နဲ့ cache lifetime ပေးနိုင်ပါတယ်။ ဒါက cookies, headers (သို့) `searchParams` တွေကို တိုက်ရိုက်ဖတ်တဲ့ function တစ်ခုကို lifetime ပေးပြီး — [prefetch](#prefetching) ထဲမှာ ပါဝင်နိုင်အောင် လုပ်ပေးပါတယ်။
...
> **သိထားသင့်သည်** — `<CachedContent />` က request data ရဲ့ နောက်မှာ ပိတ်ထားလို့ — prerendered static shell ထဲ မထည့်ပါဘူး။ Runtime မှာ default အနေနဲ့ [in-memory](https://nextjs.org/docs/app/api-reference/directives/use-cache#runtime-caching-considerations) မှာ cache လုပ်ပြီး — serverless requests တွေကြားမှာ မတည်မြဲလို့ request တစ်ခုချင်းစီမှာ ပြန်အကဲဖြတ်ရနိုင်ပါတယ်။ Durable, shared caching အတွက် [`'use cache: remote'`](https://nextjs.org/docs/app/api-reference/directives/use-cache) ကို သုံးပါ။

ဒီပုံစံနဲ့ [prefetching](#prefetching) က client transition တစ်ခုအတွင်း user ရဲ့ တကယ့် session နဲ့ `<CachedContent />` ကို prerender လုပ်ပြီး — click မလုပ်ခင် ရလဒ်ကို အသင့်ဖြစ်အောင် လုပ်နိုင်ပါတယ်။ Server-side entries တွေက requests တွေကြားမှာ ခဲခဲယဉ်းယဉ်းပဲ ကျန်ရစ်တာတောင် အလုပ်လုပ်ပါတယ် — ဘာလို့လဲဆိုတော့ သင်သတ်မှတ်တဲ့ lifetime က ရလဒ်ကို prefetch ထဲ ပါဝင်စေပြီး — client က သူ့ရဲ့ [`cacheLife`](https://nextjs.org/docs/app/api-reference/functions/cacheLife) `stale` window အတွင်းမှာ fresh အဖြစ် သတ်မှတ်ထားလို့ပါ။
...
`<Suspense>` က async access တွေကို ထိန်းထားသလိုပဲ — **error boundary** က failure တွေကို ထိန်းထားပါတယ်: rendering ကာလအတွင်း error ဖြစ်နိုင်တဲ့ subtree တစ်ခုကို wrap လုပ်ပါ။ Component-level boundaries တွေအတွက် [`catchError`](https://nextjs.org/docs/app/api-reference/functions/catchError) ကို သုံးပြီး — route-level boundaries တွေအတွက် [`error.js`](https://nextjs.org/docs/app/api-reference/file-conventions/error) file convention ကို သုံးပါ။

ဆောက်ရင်းနဲ့ သတိပြုရမှာက — [`generateMetadata`](https://nextjs.org/docs/app/api-reference/functions/generate-metadata#with-cache-components) နဲ့ [`generateViewport`](https://nextjs.org/docs/app/api-reference/functions/generate-viewport#with-cache-components) အတွင်းမှာ uncached fetches (သို့) runtime data access တွေက page ထဲမှာလိုပဲ insights နဲ့ errors တွေကို ပြတာမို့ — သင်ရည်ရွယ်ထားတဲ့ rendering ဆီ လမ်းညွှန်ပေးပါတယ်။ Params value သိထားတဲ့ နဲ့ မသိတဲ့ နှစ်မျိုးလုံးနဲ့ incremental static regeneration အတွက် — [Cache Components နဲ့ ISR](https://nextjs.org/docs/app/guides/incremental-static-regeneration) ကို ကြည့်ပါ။
...
**Request တစ်ခုချင်းစီအတွက် ထူးခြားတဲ့ (unique) values တွေ ထုတ်လုပ်ဖို့** — ဒီ operations တွေ မလုပ်ခင် request time ဆီ ရွှေ့ဖို့ [`connection()`](https://nextjs.org/docs/app/api-reference/functions/connection) ကို ခေါ်ပြီး component ကို `<Suspense>` ထဲမှာ wrap လုပ်ပါ:
...
> **သိထားသင့်သည်** — ဒီထဲမှာ synchronous APIs တွေနဲ့ embedded databases တွေဆီက queries တွေ ပါဝင်ပါတယ် — ဥပမာ `better-sqlite3` (သို့) Node.js ရဲ့ built-in [`node:sqlite`](https://nodejs.org/api/sqlite.html)။ Synchronous source တစ်ခုကနေ per-request data လိုအပ်ရင် — query မလုပ်ခင် [`connection()`](https://nextjs.org/docs/app/api-reference/functions/connection) ကို ခေါ်ပါ။
...
Data ကို rendering ကာလအတွင်းမှာ တွက်ချက်ပြီး requests တွေကြားမှာ ပြန်သုံးသင့်ရင်တော့ — read ကို [`use cache`](https://nextjs.org/docs/app/api-reference/directives/use-cache) ထဲမှာ wrap လုပ်ပါ။ Data က incoming request ပေါ်မှာ မှီခိုတယ် (သို့) အချိန်နဲ့အမျှ ပြောင်းမယ်လို့ မျှော်လင့်ရရင် — request-time rendering ကာလအတွင်းမှာ ဖတ်ပါ။
...
- [`use cache`](#usage) — lifetime [သိပ်မတိုဘူးဆိုရင်](https://nextjs.org/docs/app/api-reference/functions/cacheLife#prerendering-behavior) ရလဒ်ကို cache လုပ်ပြီး static shell ထဲ ထည့်ပါတယ်
...
Route တစ်ခုရဲ့ static shell ထဲမှာ ဘာတွေ ပါဝင်လဲဆိုတာက build time မှာ သိရတဲ့အရာပေါ် မူတည်ပါတယ်။ Route တစ်ခုရဲ့ [dynamic params](https://nextjs.org/docs/app/api-reference/functions/generate-static-params) တွေကို သိတဲ့အခါ — shell ထဲမှာ အဲဒီ concrete content ပါဝင်ပြီး ကျန်တဲ့ uncached (သို့) runtime data တွေက သူတို့ရဲ့ `<Suspense>` fallback နောက်မှာ ဆက်ပြီး stream လုပ်ပါတယ်။ Params တွေကို မသိတဲ့အခါ — ပြန်သုံးလို့ရတဲ့, URL-independent version က [**App Shell**](https://nextjs.org/docs/app/glossary#app-shell) ပါ: param-specific အပိုင်းတွေက သူတို့ရဲ့ fallbacks နောက်မှာ ကျန်ရစ်ထားတဲ့ တူညီတဲ့ static shell ပဲ ဖြစ်ပါတယ်။ [Incremental Static Regeneration](#incremental-static-regeneration) က ပထမဆုံး visit အပြီးမှာ concrete versions တွေကို ဖြည့်ပေးပါတယ်။
...
ဒီ param က dynamic ဖြစ်ရင် ([`generateStaticParams`](https://nextjs.org/docs/app/api-reference/functions/generate-static-params) က မပေးထားရင်) — သူက runtime data ဖြစ်လို့ layout ကို prerender လုပ်လို့ မရပါဘူး။
...
ဥပမာတွေနဲ့ စစ်ဆေးရေး tools တွေအတွက် [Instant navigation guide](https://nextjs.org/docs/app/guides/caching) ကို ဖတ်ပါ။

### Prefetching

[Partial Prefetching](https://nextjs.org/docs/app/api-reference/next-config-js/partialPrefetching) enable လုပ်ထားရင် — router က route တစ်ခုချင်းစီရဲ့ [App Shell](https://nextjs.org/docs/app/glossary#app-shell) ကို default အနေနဲ့ prefetch လုပ်ပါတယ်။ App Shell ထဲမှာ static content တွေနဲ့ `cookies()` နဲ့ `headers()` တွေကနေ ဆင်းသက်လာတဲ့ session data တွေ ပါဝင်ပါတယ်။ Link တစ်ခုရဲ့ **URL data** — `searchParams` (သို့) dynamic `params` လိုမျိုး — ပေါ်မှာ မှီခိုတဲ့ cached content တွေကိုပါ prefetch လုပ်ချင်ရင် အဲဒီ link ပေါ်မှာ `prefetch={true}` သတ်မှတ်ပါ။

[`<Link prefetch={true}>`](https://nextjs.org/docs/app/api-reference/components/link#prefetch) က [Partial Prefetching](https://nextjs.org/docs/app/api-reference/next-config-js/partialPrefetching) route တစ်ခုကို ညွှန်နေရင် — Next.js က prefetch time မှာ အဲဒီ route ရဲ့ component tree ကို ဒီတစ်ခါ destination URL ဖြေရှင်းပြီးသားနဲ့ ပြန် render လုပ်ပါတယ်။ တူညီတဲ့ rules တွေ သက်ရောက်ပေမယ့် — ဒီတစ်ခါ `searchParams` နဲ့ `params` တွေ scope ထဲ ရောက်နေလို့ tree ရဲ့ ပိုများတဲ့ အစိတ်အပိုင်း ဖြေရှင်းနိုင်ပါတယ်:
...
- [`use cache: private`](https://nextjs.org/docs/app/api-reference/directives/use-cache) က server ပေါ်မှာ run လုပ်ပြီး runtime data တွေကို တိုက်ရိုက် ဖတ်ကာ — ရလဒ်ကို per-link prefetch ရဲ့ တစ်စိတ်တစ်ပိုင်းအနေနဲ့ browser ထဲမှာ cache လုပ်ပါတယ်
...
`/search?q=shoes` ဆီ [`<Link>`](https://nextjs.org/docs/app/api-reference/components/link) တစ်ခုကို prefetch လုပ်တဲ့အခါ — framework က link ရဲ့ URL ကနေ `searchParams` ကို ဖြေရှင်းလို့ cached `search` ရလဒ်က click မလုပ်ခင် runtime prerender ထဲ ပါဝင်သွားပါတယ်။ Browser က အဲဒါကို သူ့ရဲ့ [`stale`](https://nextjs.org/docs/app/api-reference/functions/cacheLife#stale) အချိန် ကုန်တဲ့အထိ (သို့) `searchParams` တွေ ပြောင်းသွားတဲ့အထိ ပြန်သုံးပါတယ်။

[Adopting Partial Prefetching](https://nextjs.org/docs/app/guides/caching) မှာ `<Link>` prefetching က ဘယ်လို ပြုမူလဲ၊ ဘယ်လို adopt လုပ်ရမလဲဆိုတာ နားလည်နိုင်ပါတယ်။

ပုံစံ အပြည့်အစုံအတွက် [Optimizing prefetching guide](https://nextjs.org/docs/app/guides/caching) ကို ကြည့်ပြီး — mode အားလုံးအတွက် [`prefetch` reference](https://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config#prefetch) ကို ကြည့်ပါ။

## Cache လုပ်ထားတဲ့ content တွေကို ဘယ်မှာ သိမ်းလဲ

Cached function တစ်ခုရဲ့ output ကို **RSC payload** အဖြစ် build time (သို့) runtime မှာ serialized လုပ်ပါတယ်။ တခြားအရာအားလုံး ဒီ payload ကနေ အလုပ်လုပ်ပါတယ်။ Next.js က ဒါကို HTML အဖြစ် render လုပ်ပြီး — server (သို့) remote store ထဲမှာ ထားပါတယ်၊ (သို့) browser ဆီ ပို့ပါတယ် — [`cacheLife`](https://nextjs.org/docs/app/api-reference/functions/cacheLife) က copy တစ်ခုချင်းစီ fresh ဖြစ်နေတဲ့ ကြာချိန်ကို သတ်မှတ်ပါတယ်:

- **Prerendered HTML** — Payload ကို HTML အဖြစ် render လုပ်ပြီး self-hosting လုပ်ရင် disk ပေါ်မှာ၊ (သို့) သင့် platform ရဲ့ CDN နောက်က durable storage ထဲမှာ သိမ်းပါတယ်။ အဲဒီ HTML က build time မှာ [static shell](#prerendering) ဖြစ်ပြီး [ISR](#incremental-static-regeneration) upgrade လုပ်ပြီးနောက် concrete page ဖြစ်ပါတယ် — [`revalidate`](https://nextjs.org/docs/app/api-reference/functions/cacheLife#revalidate) နဲ့ [`expire`](https://nextjs.org/docs/app/api-reference/functions/cacheLife#expire) တွေက ဘယ်အချိန်မှာ ပြန်ဆောက်မလဲ ထိန်းချုပ်ပါတယ်။
- **Shared store** — Default အနေနဲ့ ရလဒ်က per-instance, in-memory store ထဲမှာ ကျန်နေပြီး serverless ပေါ်မှာတော့ ephemeral ဖြစ်ပါတယ်။ [`use cache: remote`](https://nextjs.org/docs/app/api-reference/directives/use-cache) က ဒါကို instances တွေကြားမှာ share လုပ်ထားတဲ့ durable [cache handler](https://nextjs.org/docs/app/api-reference/next-config-js/cacheHandlers) တစ်ခုဆီ ရွှေ့ပါတယ် — network roundtrip တစ်ခု ကုန်ကျပြီး **hit rate မြင့်မှသာ** အကျိုးရှိပါတယ်။
- **Browser** — Payload က client navigation (သို့) [prefetch](#prefetching) တစ်ခုအတွက် ပို့တဲ့ RSC ထဲ ပါဝင်ပြီး — browser က သူ့ရဲ့ [`stale`](https://nextjs.org/docs/app/api-reference/functions/cacheLife#stale) window အတွင်း fresh ထားပါတယ်။ [`use cache: private`](https://nextjs.org/docs/app/api-reference/directives/use-cache) ရလဒ်တွေက ဒီနေရာမှာပဲ ရှိပါတယ်။

> **သိထားသင့်သည်** — `cookies()` (သို့) `headers()` တွေကို ဖတ်တဲ့ [App Shell](https://nextjs.org/docs/app/glossary#app-shell) တစ်ခုက session-specific ဖြစ်ပြီး — shared server cache ထဲမှာ မဟုတ်ဘဲ client ပေါ်မှာ session တစ်ခုချင်းစီအလိုက် cache လုပ်ပါတယ်။

ဒီ stores တွေအားလုံးက deployment တစ်ခုတည်းကိုပဲ scope လုပ်ထားပါတယ်။ Deploy အသစ်တစ်ခုက fresh စပြီး — prerenders အသစ်တွေ ဆောက်ပါတယ် — `use cache` entries တွေက durable [`remote`](https://nextjs.org/docs/app/api-reference/directives/use-cache) တွေတောင် ဆက်မပါပါဘူး — ဘာလို့လဲဆိုတော့ [cache key](https://nextjs.org/docs/app/api-reference/directives/use-cache#cache-keys) ထဲမှာ build id ပါဝင်လို့ပါ။ Environment တစ်ခုချင်းစီရဲ့ အပြုအမူအတွက် [Runtime caching considerations](https://nextjs.org/docs/app/api-reference/directives/use-cache#runtime-caching-considerations) ကို ကြည့်ပြီး — server cache ကို configure လုပ်ဖို့ [Self-hosting](https://nextjs.org/docs/app/guides/self-hosting#caching-and-isr) ကို ကြည့်ပါ။

## Incremental Static Regeneration

Dynamic param segments တွေပါတဲ့ route တစ်ခုမှာ — [`generateStaticParams`](https://nextjs.org/docs/app/api-reference/functions/generate-static-params) က build time မှာ သင်စာရင်းပြုစုထားတဲ့ URLs တွေကို prerender လုပ်ပါတယ်။ တခြား URL တိုင်းကို [App Shell](https://nextjs.org/docs/app/glossary#app-shell) အနေနဲ့ ချက်ချင်း ပေးပြီး — အဲဒီနောက် အခု သိပြီဖြစ်တဲ့ params တွေနဲ့ နောက်ခံမှာ upgrade လုပ်ကာ နောက် visitor အတွက် cache လုပ်ပါတယ်။

Walkthrough အပြည့်အစုံအတွက် [Cache Components နဲ့ ISR](https://nextjs.org/docs/app/guides/incremental-static-regeneration) ကို ကြည့်ပါ။
